...
loadpackage(c("plyr","dplyr","ggplot2","knitr","broom","ggfortify","future","kableExtra","xts","eegUtils","dlm","forecast","signal","promises",
...
## 100 datapoints per second
  ...
  observe({
    ...
    if(ExG$recording==T){
       ...
       promise<-future_promise({ ### EVALUATE PARALLEL
        .libPaths("C:/Users/royde/Documents/R/win-library/4.0") ### GRAB DEFAULTS
        library(reticulate) ### LOAD EM
        mainDir<<-getwd()
        source_python(paste0(mainDir,"/EEG.py")) ### CREATE LSL STREAM
  ...
  then(promise,onFulfilled = 
  function(value) {
    ExG$ts<<-rbind(ExG$ts,as.xts(data.frame(value),
                seq(last(index(ExG$ts))+1,
                    by=1/ExG$srate, length=nrow(data.frame(tmp))))/(1000*1000))
  })
    print("evaluated promise")  
  
    # then(v,onFulfilled=function(v){ 
    #           rbind(ExG$ts,
    #                        as.xts(data.frame(v),
    #             seq(last(index(ExG$ts))+1,
    #                 by=1/ExG$srate, length=nrow(data.frame(tmp))))/(1000*1000))
